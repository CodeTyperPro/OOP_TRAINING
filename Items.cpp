...
void remove(std::vector<C>& items, C item){
    items.erase(item); // Remove the given element
}
...
bool exist(std::vector<C> &items, const std::string name){
    for (C& i : items){
        if(i.getName() == name){ // If the element element match with that name, means that exist, so, return true.
            return true;
        }
    }
    return false; // Otherwise, it's false obviously.
}
...
// Check if whether the given name is present in our vector, return and index>=0 if it's okay, or -1 if not
template<typename C>
